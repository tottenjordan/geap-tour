...
def add_console_link(slide, console_url, y_pos=None):
    if y_pos is None:
        y_pos = Inches(6.9)
...
def build_deck():
    ...
    add_notes(
        s,
        "Welcome everyone to the Gemini Enterprise Agent Platform workshop. "
    ...
    s = prs.slides.add_slide(blank)
    add_text(
        s, Inches(0.8), Inches(0.3), Inches(8), Inches(0.8), "Table of Contents", 42, bold=True
    )
    add_text(
        s,
        Inches(0.8),
    ...
    for i, (color, num, title, desc) in enumerate(toc_items):
        ...
        # Title
        add_text(s, Inches(1.8), y, Inches(10), Inches(0.5), title, 24, bold=True, color=DARK)
        # Description
        add_text(
            s, Inches(1.8), Emu(y + Inches(0.45)), Inches(10), Inches(0.5), desc, 16, color=GRAY
        )
    add_logo(s)
    add_notes(
        s,
        "Four sessions today. Session 1 is the longest — we'll build the full agent stack from scratch. "
    ...
    s = prs.slides.add_slide(blank)
    add_text(
        s, Inches(0.8), Inches(0.3), Inches(8), Inches(0.8), "Platform Architecture", 42, bold=True
    )
    add_image_safe(
        s,
        os.path.join(SCREENSHOTS, "geap_architecture.png"),
        Inches(1.5),
        Inches(1.4),
        width=Inches(10),
    )
    ...
    add_notes(
        s,
        "This is the high-level architecture of GEAP. Key point: the Agent Gateway sits at the network boundary — "
    ...
    s = prs.slides.add_slide(blank)
    add_text(
        s, Inches(0.8), Inches(0.3), Inches(8), Inches(0.8), "Agent Identity Model", 42, bold=True
    )
    add_image_safe(
        s, os.path.join(SCREENSHOTS, "identity_types.png"), Inches(1), Inches(1.3), width=Inches(11)
    )
    add_logo(s)
    add_notes(
        s,
        "Three identity types are critical to understand. "
    ...
    add_bullet_list(
        ...
        font_size=20,
        color=RGBColor(0xEE, 0xEE, 0xFF),
    )
    add_repo_link(s, y_pos=Inches(6.5))
    add_logo(s, RGBColor(0x88, 0x99, 0xCC))
    add_notes(
        s,
        "Session 1 is where we build everything. By the end of this session you'll have: "
    ...
    s = prs.slides.add_slide(blank)
    add_text(
        s, Inches(0.8), Inches(0.3), Inches(8), Inches(0.8), "Multi-Agent Topology", 36, bold=True
    )
    add_image_safe(
        s,
        os.path.join(DIAGRAMS, "01_multi_agent_topology.png"),
        Inches(0.5),
        Inches(1.3),
        width=Inches(6),
    )
    ...
    add_bullet_list(
        ...
        title="Three MCP Servers",
        title_color=BLUE,
    )
    add_logo(s)
    add_notes(
        s,
        "Our workshop builds a corporate travel assistant. The Coordinator agent routes requests "
    ...
    s = prs.slides.add_slide(blank)
    add_text(
        s, Inches(0.8), Inches(0.3), Inches(8), Inches(0.8), "MCP Server Development", 36, bold=True
    )
    add_text(
        s,
        Inches(0.8),
    ...
    add_notes(
        s,
        "FastMCP makes building tool servers trivial — it's a single decorator on a Python function. "
    ...
    s = prs.slides.add_slide(blank)
    add_text(
        s, Inches(0.8), Inches(0.3), Inches(8), Inches(0.8), "ADK Agent Architecture", 36, bold=True
    )
    add_text(
        s,
        Inches(0.8),
    ...
    add_image_safe(
        s,
        os.path.join(DIAGRAMS, "02_deployment_architecture.png"),
        Inches(0.5),
        Inches(1.3),
        width=Inches(6),
    )
    ...
    add_notes(
        s,
        "Two deployment targets: Cloud Run for MCP servers (stateless HTTP services) "
    ...
    add_image_safe(
        s,
        os.path.join(SCREENSHOTS, "session1_agent_engine.png"),
        Inches(0.8),
        Inches(1.3),
        width=Inches(11.5),
    )
    ...
    add_notes(
        s,
        "CONSOLE DEMO: Show the Agent Engine console page. Point out the deployed agent, "
    ...
    add_image_safe(
        s,
        os.path.join(DIAGRAMS, "04_agent_identity_gateway.png"),
        Inches(0.5),
        Inches(1.3),
        width=Inches(6),
    )
    ...
    add_notes(
        s,
        "SPIFFE gives every agent a cryptographic identity — not just a service account. "
    ...
    add_notes(
        s,
        "Walk through the gcloud commands to create a workload identity pool and OIDC provider. "
    ...
    add_notes(
        s,
        "Session 2 builds on the deployed system from Session 1. "
    ...
    add_table(
        ...
        [
            ...
            [
                "Agent-to-Anywhere",
                "geap-workshop-gateway-egress",
                "Gemini model calls, MCP tools, external APIs",
            ],
        ],
        font_size=13,
    )
    ...
    add_image_safe(
        s,
        os.path.join(SCREENSHOTS, "session2_agent_gateway.png"),
        Inches(7),
        Inches(1.3),
        width=Inches(5.8),
    )
    ...
    add_notes(
        s,
        "The dual-mode gateway is a key differentiator. Ingress gateway controls who can talk to your agents. "
    ...
    add_image_safe(
        s,
        os.path.join(SCREENSHOTS, "session2_agent_traces.png"),
        Inches(0.8),
        Inches(1.3),
        width=Inches(11.5),
    )
    ...
    add_notes(
        s,
        "CONSOLE DEMO: Show the Cloud Trace view of an agent interaction. "
    ...
    add_table(
        ...
        [
            ...
            [
                "Model Armor",
                "Content screening",
                "Prompt injection, jailbreak, PII, harmful content",
            ],
        ],
        font_size=13,
    )
    ...
    add_notes(
        s,
        "Three layers of governance, each catching different problems. "
    ...
    add_text(
        ...
        Inches(10),
        Inches(0.8),
        "Delegating Authorization to the Gateway",
        36,
        bold=True,
    )
    add_text(
    ...
    add_notes(
        s,
        "This slide explains the WIRING — how you connect IAP, Model Armor, and SGP to the gateway. "
    ...
    add_image_safe(
        s,
        os.path.join(DIAGRAMS, "03_eval_pipeline.png"),
        Inches(3.8),
        Inches(1.1),
        height=Inches(3),
    )
    ...
    add_notes(
        s,
        "Three evaluation tiers, each serving a different purpose. "
    ...
    add_text(
        ...
        Inches(10),
        Inches(0.8),
        "Console: Evaluation Experiments",
        32,
        bold=True,
        color=WHITE,
    )
    add_image_safe(
        s,
        os.path.join(SCREENSHOTS, "session2_evaluation.png"),
        Inches(0.8),
        Inches(1.3),
        width=Inches(11.5),
    )
    ...
    add_notes(
        s,
        "CONSOLE DEMO: Show the Evaluation Experiments page. "
    ...
    s = prs.slides.add_slide(blank)
    add_text(
        s, Inches(0.8), Inches(0.3), Inches(8), Inches(0.8), "Observability Stack", 36, bold=True
    )
    add_image_safe(
        s,
        os.path.join(DIAGRAMS, "05_observability_stack.png"),
        Inches(0.5),
        Inches(1.3),
        width=Inches(6),
    )
    ...
    add_notes(
        s,
        "The observability pipeline: agent calls produce OTel spans automatically (built into ADK), "
    ...
    add_card(
        ...
        "Each cluster includes sample count, avg score, and representative examples.",
        BLUE,
    )
    add_card(
        s,
        ...
        "10-minute aggregation window with custom metric: agent_eval/{metric_name}.",
        RED,
    )
    ...
    add_notes(
        s,
        "Failure cluster analysis is the bridge between raw eval results and actionable insights. "
    ...
    add_image_safe(
        s,
        os.path.join(SCREENSHOTS, "session2_cloud_trace.png"),
        Inches(0.8),
        Inches(1.3),
        width=Inches(11.5),
    )
    ...
    add_notes(
        s,
        "CONSOLE DEMO: Open Cloud Trace and show a real agent trace. "
    ...
    add_image_safe(
        s, os.path.join(DIAGRAMS, "06_ci_cd_flow.png"), Inches(0.5), Inches(1.3), width=Inches(6)
    )
    add_text(
        s,
        Inches(7),
    ...
    add_notes(
        s,
        "The CI/CD eval gate is the automated enforcement layer. "
    ...
    add_text(
        ...
        Inches(10),
        Inches(0.8),
        "Agent Optimization — GEPA Algorithm",
        36,
        bold=True,
    )
    add_bullet_list(
        ...
        title="Gemini Evolutionary Prompt Algorithm",
        title_color=BLUE,
    )
    add_text(
        s,
    ...
    add_notes(
        s,
        "GEPA — Gemini Evolutionary Prompt Algorithm — automates prompt engineering. "
    ...
    s = prs.slides.add_slide(blank)
    add_text(
        s, Inches(0.8), Inches(0.3), Inches(10), Inches(0.8), "Multi-Model Router", 36, bold=True
    )
    add_text(
        s,
        Inches(0.8),
    ...
    add_notes(
        s,
        "Memory Bank gives agents persistent memory across sessions. Two key pieces: "
    ...
    add_text(
        ...
        22,
        color=RGBColor(0x9A, 0xA0, 0xA6),
    )
    add_repo_link(s, y_pos=Inches(6.5))
    add_logo(s, RGBColor(0x88, 0x88, 0x88))
    add_notes(
        s,
        "Quick session on Agent Registry — the catalog for your agent fleet. "
    ...
    add_image_safe(
        s,
        os.path.join(SCREENSHOTS, "session3_agent_registry_mcp.png"),
        Inches(7),
        Inches(1.3),
        width=Inches(5.8),
    )
    ...
    add_notes(
        s,
        "Agent Registry provides four capabilities: registration (with metadata and versioning), "
    ...
    add_image_safe(
        s,
        os.path.join(SCREENSHOTS, "session3_policies.png"),
        Inches(0.8),
        Inches(1.3),
        width=Inches(11.5),
    )
    add_console_link(
        s, f"https://console.cloud.google.com/agent-platform/gateways?project={GCP_PROJECT}"
    )
    add_logo(s, RGBColor(0x88, 0x88, 0x88))
    add_notes(
        s,
        "CONSOLE DEMO: Show the business policies page. "
    ...
    add_notes(
        s,
        "Final session — the security layer. Model Armor provides content safety for your agent system. "
    ...
    add_image_safe(
        s, os.path.join(DIAGRAMS, "07_agent_armor.png"), Inches(0.5), Inches(1.3), width=Inches(6)
    )
    add_card(
        s,
        Inches(7),
    ...
    add_notes(
        s,
        "Model Armor works at two points: input screening (before the model call) and output screening (after). "
    ...
    add_image_safe(
        s,
        os.path.join(SCREENSHOTS, "session4_model_armor.png"),
        Inches(0.8),
        Inches(1.3),
        width=Inches(11.5),
    )
    add_console_link(
        s, f"https://console.cloud.google.com/security/model-armor?project={GCP_PROJECT}"
    )
    add_logo(s, RGBColor(0x88, 0x88, 0x88))
    add_notes(
        s,
        "CONSOLE DEMO: Show the Model Armor configuration page. "
    ...
    # ===== SLIDE 27: SUMMARY =====
    s = prs.slides.add_slide(blank)
    add_text(s, Inches(0.8), Inches(0.3), Inches(8), Inches(0.8), "Platform Summary", 42, bold=True)
    cards = [
        (
            "Build",
            ["ADK agents with LlmAgent", "FastMCP tool servers", "Memory Bank + Router"],
            BLUE,
        ),
        (
            "Deploy",
            ["Cloud Run (MCP servers)", "Agent Runtime (agents)", "One-command deployment"],
            GREEN,
        ),
    ...
    add_notes(
        s,
        "Recap the six pillars: Build (ADK + MCP), Deploy (Cloud Run + Agent Runtime), "
    ...
    add_notes(
        s,
        "Share these resources with attendees. The workshop repo has all the code they ran today. "
