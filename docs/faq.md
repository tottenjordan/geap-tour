...
**What is it?** — A three-tier evaluation system covering batch testing, periodic-snapshot monitoring via the offline-eval bridge, and an on-demand Vertex Managed Pipeline that runs the full eval DAG for the deployed agents.

**How does it work?** — The first tier is batch evaluation: `one_time_eval.py` and `batch_eval.py` run 20 test cases across 11 categories using custom `PointwiseMetric` rubrics that score safety, quality, tool use, and policy compliance. The second tier is the periodic-snapshot offline-eval bridge: because the native Vertex Online Evaluators are platform-blocked (the managed Agent Engine runtime strips prompt/response content from OTel traces, so they always return `INSUFFICIENT_DATA`), the bridge is the canonical quality source instead. It scores the deployed engine via the Vertex Gen AI Evaluation Service and writes point-in-time snapshots to two honest, separate surfaces: `publish_offline_eval.py` publishes coordinator quality (`helpfulness`, `tool_use_accuracy`, `policy_compliance`) to `custom.googleapis.com/agent_eval/*` on a 1-5 rubric (alert `< 3.0`), and `publish_router_efficiency.py` publishes router economics (`routing_accuracy_pct`, `cost_savings_pct`, `classifier_latency_ms`) to `custom.googleapis.com/agent_router/*` in native units. Both surfaces are read back by `verify_monitors.py`. The third tier is the orchestrated eval pipeline: the whole suite (deploy → traffic → batch ‖ simulated ‖ complexity → monitor → report) runs as a single **Vertex AI Managed Pipeline** (KFP v2), submitted on demand via `src/pipelines/submit.py` (or the manual `eval_vertex.yaml` workflow), with `simulated_eval.py` gating on a 3.0 score threshold. All eval compute runs on Vertex — observable in the Vertex Pipelines UI with lineage and artifacts. Supporting tools include `failure_clusters.py` for grouping error patterns and `quality_alerts.py` for threshold-based notifications.
...
- `src/eval/publish_offline_eval.py` — Coordinator quality bridge → `agent_eval/*` (1-5 rubric snapshots)
- `src/eval/publish_router_efficiency.py` — Router efficiency bridge → `agent_router/*` (native-unit snapshots)
- `src/eval/verify_monitors.py` — Reads both monitored surfaces (coordinator_quality + router_efficiency)
- `src/eval/simulated_eval.py` — Multi-turn simulated eval (gates on score 3.0)
- `src/pipelines/eval_pipeline.py` / `src/pipelines/submit.py` — Vertex Managed Pipeline (KFP v2) DAG + manual submit CLI
- `.github/workflows/eval_vertex.yaml` — Manual `workflow_dispatch` that submits the pipeline to Vertex
